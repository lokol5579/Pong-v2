...
class DuelingDQN(nn.Module):
    def __init__(self, state_size, action_size):
        ...
        self.fc_val1 = nn.Linear(fc_input_dims, 512)
    ...
    def forward(self, state):
        ...
        val = F.relu(self.fc_val1(layer))
        val2= self.fc_val2(val)
...
class DuelingDoubleDQNAgent:
    def __init__(self, state_size, action_size, batch_size=64, gamma=0.99, lr=0.0001, memory_size=20000):
        ...
        # 创建两个网络
        self.dqn_net = DuelingDQN(self.state_size, self.action_size).to(device)
        self.target_net = DuelingDQN(self.state_size, self.action_size).to(device)
    ...
    def update(self, step):
       ...
       # 更新target_net
        ...
        nxt_q_vals_dqn = self.dqn_net(next_states)
        nxt_q_vals_target = self.target_net(next_states)

        q_val = q_vals.gather(1, actions.unsqueeze(-1)).squeeze(-1)
        # 使用dqn_net选择动作，使用target_net评估Q值
        nxt_q_val = nxt_q_vals_target.gather(1, nxt_q_vals_dqn.argmax(1).unsqueeze(-1)).squeeze(-1)
